...
namespace NewElevatorFramework
{
    class Scheduler
    {
        ...
        // This param is tuned to fit the test data
        const int minFreeCapacity = 55;
            ...
            if (ElevValid[i])
            {
                ...
                else // no true task
                {
                    ...
                    if ((directionButtonPanels[0].Buttons[Up].State == ButtonState.Pressed
                      || directionButtonPanels[1].Buttons[Up].State == ButtonState.Pressed) && ElevMode==ElevModeComeIn)
                    {
        ...
        public void despatchQueriesToElev()
        {
            ...
            //记录哪几个电梯可用和是否有真正的响应目标
            for (int i = 0; i < ElevValid.Length; i++)
            {
            ...
            if (isBug)
            {
                Utility.logWarning("all elevators are Idle when there are requests");
            }
        }
